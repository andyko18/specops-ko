...
- `SessionStart` → `session-start.sh`: 메타 스킬 주입 + session-progress rehydrate. **조립 순서 계약**: `specops-ko-anchor` → `freecomment-pending` → `session-progress-reconcile` → 메타 본문 → `session-progress-rehydrate` — harness 가 큰 additionalContext 를 선두 일부만 인라인하므로 행동 지시 블록을 앞에 둔다 (`scripts/tests/test-session-start-order.sh` 가 잠금)
