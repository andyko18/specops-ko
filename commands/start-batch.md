...
4. **batch 브랜치 생성** (1회):
   ...
   ```
5. **queue.md 초기화** — `.specops/$BATCH_ID/queue.md` 생성:
   ```
   | FR-ID | FID | FR 설명(1줄) | Status |
   |---|---|---|---|
   | FR-1 | YYYYMMDD-fr-1 | <FR 설명> | PENDING |
   | FR-2 | YYYYMMDD-fr-2 | <FR 설명> | PENDING |
   ...
   ```
   FID 슬러그: `FR-1` → `fr-1`, `FR-2` → `fr-2` (소문자·하이픈 치환)
...
4. decomposing-ko 출력에서 `BATCH-PHASE1-DONE: <FID>` 감지 → queue.md 해당 FR을 `PLAN_DONE` 갱신
5. 다음 PENDING FR 반복
