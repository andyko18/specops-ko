...
" 2>/dev/null)"
  ...
  #   현행 0건 / M1 변이(`rm -rf $...X` 원복) 단독 **4건** / M5 변이(`eval $...X`) 단독 **2건**.
  #   (종전 주석은 M5 를 6건으로 적었으나 그것은 M1+M5 **동시 적용값**이다 — 부모가 M1 변이본을
  #    다시 고쳐 M5 를 만든 탓에 두 변이가 겹친 측정치를 M5 단독으로 라벨했다. Phase C 재판정 적발.)
