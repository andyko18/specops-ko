...
# AC-2: 7종 모두 등장
miss=""
for s in '/start ' '/start-auto' '/start-foundation' '/start-batch' '/start-project' '/maintain' '/brainstorming'; do
...
grep -qE '/start-project →.*/start-foundation →.*/start-batch' "$R" && ok "AC-4 선택 순서" || nope "AC-4" "순서 흐름 없음"
