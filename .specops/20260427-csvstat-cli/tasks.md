# Tasks — 20260427-csvstat-cli

## AC → Task 매핑

| AC | must/should | Task(s) |
|---|---|---|
| AC-1 파일 인자 통계 출력 | must | T3 |
| AC-2 stdin 입력 지원 | must | T3 |
| AC-3 존재하지 않는 파일 | must | T3 |
| AC-4 인자·stdin 없음 → usage | must | T3 |
| AC-5 pytest PASS | must | T3 |
| AC-6 test-conventions-python.md | must | T1 |
| AC-7 decomposing-ko HARD-GATE Python 분기 | must | T2 |
| AC-8 exec-bit + shebang (csvstat.py) | should | T3 |
| AC-9 빈 CSV → rows: 0 | must | T3 |
...
**AC 매핑**: AC-6
**파일**:
- Create: `templates/test-conventions-python.md`

*(문서 태스크 — TDD 적용: RED=파일 없음 확인, GREEN=파일 생성)*
...
실행: `ls templates/test-conventions-python.md`
예상: exit 1
...
```bash
ls templates/test-conventions-python.md && grep -c "Universal 강제" templates/test-conventions-python.md
```
예상: 파일 존재 + `2` 이상
...
*(문서 태스크 — TDD 적용: RED=Python 언급 없음 확인, GREEN=3곳 수정)*
...
**수정 1**: HARD-GATE 섹션 — bash 테스트 파일 규약 문단 바로 뒤에 한 줄 추가.

현재 HARD-GATE의 bash 규약 줄 (line 21 부근):
```
...
**수정 3**: "테스트 컨벤션 (bash)" 섹션 — 제목 변경 + Python 서브섹션 추가.
...
그리고 bash 섹션 마지막 줄 (`상세 규약·예시 코드블록·회귀 금지 체크리스트: \`templates/test-conventions-bash.md\`.`) 바로 뒤에 추가:
...
**파일**:
- Create: `examples/tests/test_csvstat.py` (no exec-bit — Python 컨벤션)
- Create: `examples/scripts/csvstat.py` (exec-bit 필요 + shebang)

*(F-12 집약: test_csvstat.py + csvstat.py 동일 파일 쌍, TDD 내부 embedded cycle)*
...
```
예상: `ModuleNotFoundError` 또는 `ERROR collecting` (csvstat.py 없음)
...
git commit -m "feat(csvstat): CSV 통계 CLI + pytest 테스트 (7th dogfood, AC-1~5, AC-8~9)"
```
...
```mermaid
graph TD
  T1[T1: test-conventions-python.md]
  T2[T2: decomposing-ko HARD-GATE]
  T3[T3: csvstat.py + test_csvstat.py]
```
...
tasks:
  - id: T1
    depends_on: []
    inputs: []
    outputs: [templates/test-conventions-python.md]
    ac: [AC-6]
  - id: T2
    depends_on: []
    inputs: []
  ...
  - id: T3
    depends_on: []
    inputs: []
    outputs: [examples/scripts/csvstat.py, examples/tests/test_csvstat.py]
    ac: [AC-1, AC-2, AC-3, AC-4, AC-5, AC-8, AC-9]
```
