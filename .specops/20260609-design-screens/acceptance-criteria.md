...
### AC-8: 기능 설명 부족 시 최소 예시 목록 제안 후 편집 게이트

**Given** `/design-screens` 에 기능 설명이 너무 짧거나 화면 목록 자동 판단이 어려운 경우

**When** Step 1 화면 목록 도출 단계에서

**Then** 일반적인 최소 예시 목록(예: 목록·상세·입력폼 등)을 먼저 제안하고 사용자가 편집 게이트를 통해 확정한다

**검증 방법**: `grep '예시\|fallback\|최소' commands/design-screens.md | wc -l` → 1 이상
**관련 FR**: FR-1, FR-2
**우선순위**: should
