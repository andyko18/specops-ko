...
harness 600s 상한을 넘겨 포그라운드 완주가 불가능하고, R-1 이 안내하는 "백그라운드 + Read 회수" 경로가
**판정 로직에 없다**(2회 시도 모두 거부, 검증 자체는 `PASS=159 · VERIFY: PASS`). 정직한 실행이 막히는
false-deny 이고 이 PR 이 고치는 병과 같은 클래스다 — 별건 FID 로 즉시 착수한다.
